function [G,yh] = alg_4(t, yt, B, Order, S0, S1, G, yh, yp, K, X, W, S1h,nt,BO)
...
while( G(yp(t-1),t-1) > 0 )
    ...
    if (S1(yh(t-1),yt,t-1) == 0)
        S1(yh(t-1),yt,t-1) = W(W0:W1)' * basis(X,yt-1,yh(t-1)-1,t-1,1,nt);
    end
    if (S1(yh(t-1),yt,t-1) == 0)
        S1(yp(t-1),yt,t-1) = W(W0:W1)' * basis(X,yt-1,yp(t-1)-1,t-1,1,nt);
    end
...
while ( G(yh(t-1),t-1) + S1(yh(t-1),yt,t-1) < B(t-1) + S0(yp(t-1),t-1) + S1h )
    ...
    if (t-1 > 1)
        [G,yh] = alg_4(t-1,yp(t-1),B,Order,S0,S1,G,yh,yp,K,X,W,S1h,nt,BO);
    end
